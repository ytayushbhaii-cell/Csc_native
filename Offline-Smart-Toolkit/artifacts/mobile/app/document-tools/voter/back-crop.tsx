...
export default function VoterBackCropScreen() {
  ...
  return (
    <ToolScreenLayout title="Voter ID Back Crop" subtitle="Crop back side to 85.6×54mm standard size" iconName="card-account-details" color={COLOR} onReset={reset}>
      {error && <StatusBanner type="error" message={error} />}

      {!result ? (
        <>
          <DocUploadWidget file={file} onPicked={setFile} onError={setError} color={COLOR} accept="image" label="Upload Voter ID Back Image" />
          ...
          {file && (
            ...
            >
              ...
              <Text style={[styles.btnText, { color: '#fff', fontFamily: 'Inter_600SemiBold' }]}>
                {processing ? 'Processing…' : 'Crop Back to Voter ID Size'}
              </Text>
            </TouchableOpacity>
          )}
          ...
          <View style={[styles.infoBox, { backgroundColor: COLOR + '12', borderColor: COLOR + '30', borderRadius: colors.radius }]}>
            ...
            <Text style={[styles.infoText, { color: colors.foreground, fontFamily: 'Inter_400Regular' }]}>
              Crops the back side of a Voter ID card to standard EPIC dimensions: 85.6 × 54mm (1012 × 638px at 300 DPI).
            </Text>
          </View>
        </>
      ) : (
        <>
          <View style={[styles.resultBox, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
            ...
            <View style={styles.resultRow}>
              ...
              <Text style={[styles.resultVal, { color: colors.foreground, fontFamily: 'Inter_600SemiBold' }]}>85.6 × 54 mm (Back)</Text>
            </View>
            <View style={styles.resultRow}>
          ...
          <DocResultActions uri={result.uri} fileName="voter-back-cropped.jpg" color={COLOR} onReset={reset} mimeType="image/jpeg" />
        </>
      )}
    </ToolScreenLayout>
  );

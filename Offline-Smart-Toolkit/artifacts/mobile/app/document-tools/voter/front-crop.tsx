...
export default function VoterFrontCropScreen() {
  ...
  return (
    <ToolScreenLayout title="Voter ID Front Crop" subtitle="Crop front side to 85.6×54mm standard size" iconName="card-account-details-outline" color={COLOR} onReset={reset}>
      {error && <StatusBanner type="error" message={error} />}

      {!result ? (
        <>
          <DocUploadWidget file={file} onPicked={setFile} onError={setError} color={COLOR} accept="image" label="Upload Voter ID Front Image" />
          ...
          {file && (
            ...
            >
              ...
              <Text style={[styles.btnText, { color: '#fff', fontFamily: 'Inter_600SemiBold' }]}>
                {processing ? 'Processing…' : 'Crop Front to Voter ID Size'}
              </Text>
            </TouchableOpacity>
          )}
          ...
          <View style={[styles.infoBox, { backgroundColor: COLOR + '12', borderColor: COLOR + '30', borderRadius: colors.radius }]}>
            ...
            <Text style={[styles.infoText, { color: colors.foreground, fontFamily: 'Inter_400Regular' }]}>
              Crops the front side of a Voter ID card to standard EPIC dimensions: 85.6 × 54mm (1012 × 638px at 300 DPI).
            </Text>
          </View>
        </>
      ) : (
        <>
          <View style={[styles.resultBox, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
            ...
            <View style={styles.resultRow}>
              ...
              <Text style={[styles.resultVal, { color: colors.foreground, fontFamily: 'Inter_600SemiBold' }]}>85.6 × 54 mm (Front)</Text>
            </View>
            <View style={styles.resultRow}>
          ...
          <DocResultActions uri={result.uri} fileName="voter-front-cropped.jpg" color={COLOR} onReset={reset} mimeType="image/jpeg" />
        </>
      )}
    </ToolScreenLayout>
  );
